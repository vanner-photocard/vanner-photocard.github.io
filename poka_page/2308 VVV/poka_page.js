...
function pokaAttacher(pokaArray, pokaName, imgFolderAdd, scnName) {
    ...
    for (let p = 0; p < POKA_ARR.length; p++) {
        ...
        console.log(pokaDivID, pokaName, pokaMember, pokaNumber, MEM_ARR)
        ...
        for (let i = 1; i < pokaNumber + 1; i++) {
            console.log(pokaNumber, i)

            MEM_ARR.forEach((m) => {
                img_html = `<img class="poka" src="${imgFolder}${pokaDivID} ${m} (${i}).jpg" onclick="changeImage(this)" onError="this.src='image/VPA WATERMARK_.png';">`
                console.log(m, img_html)
